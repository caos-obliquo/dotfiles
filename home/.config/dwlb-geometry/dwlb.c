...
static void write_bar_geometry(uint32_t middle_x, uint32_t middle_width, uint32_t bar_height) {
	FILE *f = fopen("/tmp/dwlb-geometry", "w");
	if (f) {
		fprintf(f, "%u %u %u\n", middle_x, middle_width, bar_height);
		fclose(f);
	}
}
...
draw_frame(Bar *bar)
{
	...
	write_bar_geometry(middle_start, (bar->width - status_width) - middle_start, bar->height);
