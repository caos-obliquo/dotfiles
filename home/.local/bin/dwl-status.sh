...
while true; do
    ...
    # zsh icon (cyan)
    zsh_display="^fg(8be9fd)󰌓^fg()"
    
    # build status - one empty space at start and end for bar edge breathing room
    echo " $vol_display | ^fg(ff5555)󰻠 $cpu%^fg() | ^fg(f1fa8c)󰍛 $mem^fg() | $zsh_display | $temp_display | $net_display | $bat_display | ^fg(bd93f9)$time^fg() "
