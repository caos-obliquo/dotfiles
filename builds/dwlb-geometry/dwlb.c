...
#define USAGE                                                                 \
  ...
  "normal\n"                                                                  \
  "	-active-color-title		title colors will use active "                       \
  "colors\n"                                                                  \
  ...
  "	-vertical-padding [PIXELS]	specify vertical pixel padding "               \
  "above and below text\n"                                                    \
  "	-active-fg-color [COLOR]	specify text color of active tags "              \
  "or monitors\n"                                                             \
  "	-active-bg-color [COLOR]	specify background color of active "             \
  "tags or monitors\n"                                                        \
  "	-occupied-fg-color [COLOR]	specify text color of occupied "               \
  "tags\n"                                                                    \
  "	-occupied-bg-color [COLOR]	specify background color of "                  \
  "occupied tags\n"                                                           \
  "	-inactive-fg-color [COLOR]	specify text color of inactive tags "          \
  "or monitors\n"                                                             \
  "	-inactive-bg-color [COLOR]	specify background color of "                  \
  "inactive tags or monitors\n"                                               \
...
static uint32_t
draw_text (char *text, uint32_t x, uint32_t y, pixman_image_t *foreground,
           pixman_image_t *foreground_mask, pixman_image_t *background,
...
static void
write_bar_geometry (uint32_t middle_x, uint32_t middle_width,
                    uint32_t bar_height, bool sel)

{
  FILE *f = fopen ("/tmp/dwlb-geometry", "w");
  if (f)
    {
      fprintf (f, "%u %u %u 0x%02x%02x%02x 0x%02x%02x%02x\n", middle_x,
               middle_width, bar_height,
               (sel ? middle_bg_color_selected : middle_bg_color).red >> 8,
               (sel ? middle_bg_color_selected : middle_bg_color).green >> 8,
               (sel ? middle_bg_color_selected : middle_bg_color).blue >> 8,
               active_fg_color.red >> 8, active_fg_color.green >> 8,
               active_fg_color.blue >> 8);
      fclose (f);
    }
}
...
draw_frame (Bar *bar)
{
          ...
          if ((!bar->sel || !active) && boxw >= 3)
            {
              /* Make box hollow */
              pixman_image_fill_boxes (
                  PIXMAN_OP_SRC, foreground, &(pixman_color_t){ 0 }, 1,
                  &(pixman_box32_t){ .x1 = x + boxs + 1,
                                     .x2 = x + boxs + boxw - 1,
                                     .y1 = boxs + 1,
                                     .y2 = boxs + boxw - 1 });
              pixman_image_fill_boxes (
                  PIXMAN_OP_SRC, foreground_mask, &(pixman_color_t){ 0 }, 1,
                  &(pixman_box32_t){ .x1 = x + boxs + 1,
                                     .x2 = x + boxs + boxw - 1,
                                     .y1 = boxs + 1,
                                     .y2 = boxs + boxw - 1 });
  ...
  draw_text (bar->status.text, bar->width - status_width, y, foreground,
             foreground_mask, background, &inactive_fg_color,
             &inactive_bg_color, bar->width, bar->height, bar->textpadding,
  ...
  write_bar_geometry (middle_start, (bar->width - status_width) - middle_start,
                      bar->height, bar->sel);
  /* Draw background and foreground on bar */
  pixman_image_composite32 (PIXMAN_OP_OVER, background, NULL, final, 0, 0, 0,
                            0, 0, 0, bar->width, bar->height);
  ...
  if (!cursor_image)
    {
      ...
      if (size == 0)
        size = 24;
      struct wl_cursor_theme *cursor_theme = wl_cursor_theme_load (
          getenv ("XCURSOR_THEME"), size * buffer_scale, shm);
      cursor_image
          ...
          for (i = 0; i < seat->bar->status.buttons_l; i++)
            {
              ...
              if (seat->pointer_button == seat->bar->status.buttons[i].btn
                  && seat->pointer_x >= status_x + seat->bar->textpadding
                                            + seat->bar->status.buttons[i].x1
                                                  / buffer_scale
                  && seat->pointer_x < status_x + seat->bar->textpadding
                                           + seat->bar->status.buttons[i].x2
                                                 / buffer_scale)
                {
...
static void
dwl_wm_tags (void *data, struct zdwl_ipc_manager_v2 *dwl_wm, uint32_t amount)
{
...
static void
dwl_wm_output_layout_symbol (void *data,
                             struct zdwl_ipc_output_v2 *dwl_wm_output,
                             const char *layout)
{
  Bar *bar = (Bar *)data;
...
setup_bar (Bar *bar)
{
  ...
  if (ipc)
    {
      bar->dwl_wm_output
          = zdwl_ipc_manager_v2_get_output (dwl_wm, bar->wl_output);
      if (!bar->dwl_wm_output)
        DIE ("Could not create dwl_wm_output");
      zdwl_ipc_output_v2_add_listener (bar->dwl_wm_output,
                                       &dwl_wm_output_listener, bar);
    }
               ...
               const char *interface, uint32_t version)
{
  if (!strcmp (interface, wl_compositor_interface.name))
  ...
  else if (!strcmp (interface, zdwl_ipc_manager_v2_interface.name))
    {
      if (ipc)
        {
          dwl_wm = wl_registry_bind (registry, name,
...
set_bottom (Bar *bar)
{
  if (!bar->hidden)
    {
      zwlr_layer_surface_v1_set_anchor (
          bar->layer_surface, ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM
                                  | ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT
                                  | ZWLR_LAYER_SURFACE_V1_ANCHOR_RIGHT);
      bar->redraw = true;
    }
              ...
              if (*p != '^')
                {
                  ...
                  *end = '\0';
                  ...
                  if (!strcmp (p, "bg"))
                    {
                      Color *color;
                      ARRAY_APPEND (ct->colors, ct->colors_l, ct->colors_c,
                                    color);
                      if (!*arg)
                  ...
                  else if (!strcmp (p, "lm"))
                    {
                      if (left_button)
                        ...
                        {
                          ARRAY_APPEND (ct->buttons, ct->buttons_l,
                                        ct->buttons_c, left_button);
                          left_button->btn = BTN_LEFT;
                          snprintf (left_button->command,
                                    sizeof left_button->command, "%s", arg);
                          left_button->x1 = x;
                        }
                    }
                  else if (!strcmp (p, "mm"))
                    {
                      if (middle_button)
                        ...
                        {
                          ARRAY_APPEND (ct->buttons, ct->buttons_l,
                                        ct->buttons_c, middle_button);
                          middle_button->btn = BTN_MIDDLE;
                          snprintf (middle_button->command,
                                    sizeof middle_button->command, "%s", arg);
                          middle_button->x1 = x;
                        }
                    }
                  else if (!strcmp (p, "rm"))
                    {
                      if (right_button)
                        ...
                        {
                          ARRAY_APPEND (ct->buttons, ct->buttons_l,
                                        ct->buttons_c, right_button);
                          right_button->btn = BTN_RIGHT;
                          snprintf (right_button->command,
                                    sizeof right_button->command, "%s", arg);
                          right_button->x1 = x;
                        }
                    }
                  else if (!strcmp (p, "us"))
                    {
                      if (scrollup_button)
                        ...
                        {
                          ARRAY_APPEND (ct->buttons, ct->buttons_l,
                                        ct->buttons_c, scrollup_button);
                          scrollup_button->btn = WheelUp;
                          snprintf (scrollup_button->command,
                                    sizeof scrollup_button->command, "%s",
                                    arg);
...
read_socket (void)
{
  ...
  if (!strcmp (wordbeg, "status"))
  ...
  else if (!strcmp (wordbeg, "set-top"))
    {
      if (all)
        {
          wl_list_for_each (bar, &bar_list, link) if (bar->bottom)
              set_top (bar);
        }
      else
        {
  ...
  for (; i < argc; i++)
    {
      if (!strcmp (argv[i], "-status"))
      ...
      else if (!strcmp (argv[i], "-toggle-visibility"))
        {
          ...
          client_send_command (&sock_address, argv[i], "toggle-visibility",
                               NULL, target_socket);
          return 0;
        }
